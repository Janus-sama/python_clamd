...
try:
    __version__ = metadata.version("pyclamd")
...
class ClamdNetworkSocket(object):
    ...
    :params str host: machine host default localhost
    :params int port: clamd port default 3310
    :params int timeout: socket timeout default None

    """

    def __init__(self, host: str = '127.0.0.1', port: int = 3310, timeout: float | None = None):
        """
        class initialization

        :params str host: hostname or ip address
        :params int port: TCP port
        :params float|None timeout: socket timeout
        """
    ...
    def _init_socket(self) -> None:
        ...
        """
        try:
            self.clamd_socket = socket.socket(
    ...
    def _file_system_scan(self, command, file):
        ...
        This function performs a filesystem scan using ClamAV,
        does not stop on errors or virus detection, and supports archives.

        :param str command: The ClamAV command to execute.
        :param str file: filename or directory (MUST BE ABSOLUTE PATH !)

        :return dict: `{filename1: ('FOUND', 'virusname'), filename2: ('ERROR', 'reason')}`

        :raise ConnectionError: If there is a communication issue during scanning.
        """
    ...
    def instream(self, buff):
        ...
        :params file buff: buffer to scan

        :return dict: `{filename1: ("virusname", "status")}`:

        :raises BufferTooLongError: if the buffer size exceeds clamd limits
    ...
    def _send_command(self, cmd, *args):
        ...
        cmd = 'n{cmd}{args}\n'.format(
            cmd=cmd, args=concat_args).encode('utf-8')
    ...
    def _recv_response(self):
        ...
        try:
            ...
            raise ConnectionError(
                "Error while reading from socket: {0}".format(e.args))
...
class ClamdUnixSocket(ClamdNetworkSocket):
    ...
    def __init__(self, path: str = "/var/run/clamav/clamd.ctl", timeout: float | None = None):
        """
        class initialisation

        :params str path: unix socket path
        :params float|None timeout: socket timeout
        """
